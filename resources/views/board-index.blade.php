...
<div class="board-header">
  <div class="board-name text-align-center"><b>/{{ $boardConfig['uri'] }}/</b> - {{ $boardConfig['name'] }}</div>
...
{{--  <div class="thread-bullets">•</div> --}} 
  ...
  <div data-unix-date="{{ $thread['createdAt'] }}" class="thread-date">{{ date('Y-m-d h:i:s' , strtotime($thread['createdAt'])) /* remember to convert this to human readable */ }}</div>
...
<div class="global-util__info-box board-index__stats">
  ...
  <table class="generic-table-one">
    <tbody>
      ...
      <tr class="generic-table-one__tr">
        <td>{{ date('Y/m/d', strtotime($boardConfig['createdAt'])) }}</td>
        <td>{{ $boardConfig['isFrozen'] ? 'yes' : 'no' }}</td>
        <td>{{ $boardConfig['isSecret'] ? 'yes' : 'no' }}</td>
        <td>n/a</td>
        <td>n/a</td>
      </tr>
    </tbody>
