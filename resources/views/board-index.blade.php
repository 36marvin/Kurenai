...
<body style="margin: 0" class="body-flexbox-footer board-index-body body-background-theme">
@include('global.warnings')
@include('global.navbar')
<div class="board-header">
  <div class="board-name text-align-center"><b>/{{ $boardConfig['board_uri'] }}/</b> - {{ $boardConfig['board_name'] }}</div>
...
@if (Auth::check())
<div class="thread-post-form-wrapper index-container-basic">
...
<div class="global-util__info-box board-index__stats">
  <p class="text-align-center">Board Info</p>
  <table class="generic-table-one">
    <tbody>
      <tr class="generic-table-one__tr">
        <th>created at</th>
        <th class="extra-horizontal-padding-table">is frozen</th>
      ...
      <tr class="generic-table-one__tr">
        <td>{{ date('Y/m/d', strtotime($boardConfig['created_at'])) }}</td>
        <td>{{ $boardConfig['is_frozen'] ? 'yes' : 'no' }}</td>
        <td>{{ $boardConfig['is_secret'] ? 'yes' : 'no' }}</td>
        <td>n/a</td>
        <td>n/a</td>
      </tr>
    </tbody>
